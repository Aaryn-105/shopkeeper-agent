"""Node: recall_metric (4.2.3 / V1.0 phase 6.3).

V1.0 phase 6.3 spec:
  - [新增] 先调 LLM（extend_keywords_for_metric_recall.prompt）扩展关键词（限 ≤6 个）
  - 合并 state.keywords
  - embedding_local.encode([query, *extended_keywords]) -> 对每条向量
    faiss_client.search_metric(vec, top_k=10)
  - 按 score 排序去重，最终保留 ≤10
  - 写 state.retrieved_metrics

Note: prior implementation called FAISS.recall_metric directly without LLM
extension. V1.0 phase 6.3 introduces a keyword-expansion step (mirroring 6.2)
so the recall sees richer semantic context for metric names like GMV / AOV.
"""
...
# V1.0 phase 6.3: extend_keywords_for_metric_recall returns ≤6 keywords
MAX_EXTENDED_KEYWORDS: int = 6
# V1.0 phase 6.3: top_k=10 per vector; final cap = 10
TOPK_PER_VECTOR: int = 10
FINAL_CAP: int = 10

_PROMPT_PATH = Path(__file__).resolve().parents[2] / "prompt" / "extend_keywords_for_metric_recall.prompt"
_FALLBACK_PROMPT = (
    "扩展最多 6 个用于数据库指标召回的关键词。\n"
    "用户问题：{query}\n"
    "返回 JSON 对象："
)
...
async def _extend_keywords_via_llm(runtime, query: str) -> list[str]:
    """Step 1 of V1.0 6.3: ask the LLM to expand keywords for metric recall.
    ...
    try:
        ...
        if runtime.metrics is not None:
            runtime.metrics.record_llm_call(LLMCallStat(
                node_name="recall_metric_extend",
...
def _search_one_vector(runtime, vec: list[float], top_k: int) -> list[dict[str, Any]]:
    """Run a single FAISS .search() against metric_info; returns [] on any failure."""
    if runtime is None or runtime.faiss is None:
        return []
    coll = getattr(runtime.faiss, "metric_info", None)
    if coll is None:
        return []
    try:
        if coll.is_indexed:
...
async def recall_metric(state: AgentState, config: RunnableConfig | None = None) -> dict:
    t0 = now_ms()
    ...
    # --- Step 3: encode + per-vector FAISS search (top_k=10) -------------------
    ...
    log_node(
        "recall_metric", request_id, "ok",
        hits=len(out), extended=len(extended), texts=len(texts),
    )
    return {
        "retrieved_metrics": out,
        "node_history": history_append(
            state, "recall_metric", "ok", now_ms() - t0,
